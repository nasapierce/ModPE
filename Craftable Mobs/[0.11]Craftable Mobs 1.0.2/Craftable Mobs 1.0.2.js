...
CraftableMobs.version = "1.0.1 Indev";
...
Item.addShapedRecipe(ghastId,3,0,["ggg","hhh","ggg"],["g",367,0,"h",ghastTearId,0]); //Ghast
...
Item.addCraftRecipe(soulId,1,0,[264,1,0,enderEyeId,1,0]); //Soul
...
function useItem(x,y,z,itemId,blockId,side,itemData,blockData){
	...
	if(itemId==soulId && soul.EntityType){
		var mob = Level.spawnMob(x,y+1,z,soul.EntityType,soul.skin);
		Entity.setHealth(mob,soul.health);
		Entity.setNameTag(mob,soul.nameTag);
		Entity.setRenderType(mob,soul.renderType);
		Entity.setRot(mob,soul.pitch,soul.yaw);
		soul.EntityType = null;
		soul.health = null;
		soul.skin = null;
		soul.nameTag = null;
		soul.renderType = null;
		soul.pitch = null;
		soul.yaw = null;
	}
}
...
function attackHook(a, v){
	...
	if(Player.getCarriedItem()==soulId && !soul.EntityType){
		preventDefault();
		soul.EntityType = Entity.getEntityTypeId(v);
		soul.health = Entity.getHealth(v);
		soul.skin = Entity.getMobSkin(v);
		soul.nameTag = Entity.getNameTag(v);
		soul.renderType = Entity.getRenderType(v);
		soul.pitch = Entity.getPitch(v);
		soul.yaw = Entity.getYaw(v);
		Entity.remove(v);
		ModPE.showTipMessage("Make sure to place before exit");
	}
...
function deathHook(m, v){
	...
	if(Entity.getEntityTypeId(v)==EntityType.SLIME){
		var drop = Math.floor(Math.random()*2);
		if(drop==1){
			Level.dropItem(Entity.getX(v),Entity.getY(v),Entity.getZ(v),0,341,1,0);
		}
	}
	if(Entity.getEntityTypeId(v)==EntityType.LAVA_SLIME){
		var drop = Math.floor(Math.random()*2);
		if(drop==1){
			Level.dropItem(Entity.getX(v),Entity.getY(v),Entity.getZ(v),0,378,1,0);
		}
	}
