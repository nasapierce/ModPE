/*Ex Nihilo PE v4.0.d for Mcpe v0.9.5 optimized for BlockLauncher 1.7.9
...
TODO::
commands, dirt + sieve = acacia seeds, endstone dust, 
nether rack dust, spruce sieve, birch sieve, jungle sieve, 
crucible
*/
...
Block.defineBlock(180,"barrel",b,5,true,0); 
Block.defineBlock(181,"water_barrel",wb,5,true,0); 
//lava can only go in stone barrels
Block.defineBlock(183,"stone_barrel",sb,1);
Block.defineBlock(184,"sieve_oak",[[p,0],["crafting_table",0],[p,0],[p,0],[p,0],[p,0]],5);
...
var dirt = [464,464,464,464,295,465,362,361,466,468,467,469,471,470];
...
function useItem(x, y, z, item, block, side, data) {
 ...
 if(item==466) {
  if(block==12||block==2||block==3) {
   if(water(x, y, z)==8||water(x, y, z)==9) {
    seeding(x, y, z, 83, 0);
   }
  }
 }
...
function destroyBlock(x, y, z, side) {
 ...
 if(block==90) {
  preventDefault();
  Level.destroyBlock(x+0.5, y, z+0.5, false);
...
//clientMessage(message+"Welcome to ExNihilo PE use /help for help");
} 
//leaving the level
function leaveGame() {
...
//scanning for water used for sugarcane seeds
function water(x, y, z) {
 Level.getTile(x, y, z-1);
 Level.getTile(x-1, y, z);
 Level.getTile(x+1, y, z);
 Level.getTile(x, y, z+1);
} 
